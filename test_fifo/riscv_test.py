...
class TB:
    def __init__(self, Instruction_list, dut, dut_PC, dut_regfile):
        ...
        self.TX_DATA = [0, 1,0,1,0,0,0,1, 1]
    ...
    async def run_test(self):
        ...
        while(True):
            ...
            print("The value at x20 => ", self.dut.my_datapath.r_file.Reg_Out[20].value.integer)
        
            
            print("\nFIFO before write: ", self.dut.my_datapath.fifo_.cir_buffer.value)
                
            written = random.randint(0, 255)
            print("A new write to circular buffer : ", written)
            self.dut.my_datapath.UARTReadData.value = written
            self.dut.my_datapath.DataReadFromLine.value = 1   # it should be done by UART NORMALLY

            await RisingEdge(self.dut.clk)
            await FallingEdge(self.dut.clk)
            self.dut.my_datapath.DataReadFromLine.value = 0
            
            print("FIFO after write: ", self.dut.my_datapath.fifo_.cir_buffer.value)
            print("The value at x20 => ", self.dut.my_datapath.r_file.Reg_Out[20].value.integer)
